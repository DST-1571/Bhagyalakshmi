package com.sourceedge.bhagyalakshmi.orders.support;

/**
 * Created by Centura User1 on 08-12-2016.
 */

public class Class_Urls {
   /* public static final String Parent=" http://bhagyalakshmi.azurewebsites.net/api/bhagyalakshmi/";*/
    public static final String Parent1= "http://61.12.71.74/tulasi/appinit/";
    public static final String Login=Parent1+"FileToSendDetails.php?type=Login";
    public static final String Distributor1=Parent1+"FileToGetDetails.php?type=Distributor";
    public static final String Category=Parent1+"FileToGetCat.php?type=Category";
    public static final String Product1=Parent1+"FileToGetCat.php?type=Product";
    public static final String ProductPrice=Parent1+"FileToGetCat.php?type=ProductPrice";
    public static final String Order1=Parent1+"FileToViewOrder.php?type=Order";
    public static final String PlaceOrder1=Parent1+"FileToPlaceOrder.php?type=PlaceOrder";
    public static final String PlaceOrderDraft=Parent1+"FileToPlaceOfflineOrder.php?type=PlaceOrder";
    public static final String Company=Parent1+"FileToGetCompany.php?type=Company";

















    public static final String Parent="http://192.168.0.144:81/api/BhagyaLakshmi/";
 /*   public static final String Login=Parent+"login";*/
    public static final String Distributor=Parent+"distributor";
    public static final String DistributorId=Parent+"distributor/id";
    public static final String Retailer=Parent+"retailer";
    public static final String RetailerId=Parent+"retailer/id";
    public static final String Product=Parent+"product";
    public static final String Stock=Parent+"stock";
    public static final String Catagory=Parent+"Category";
    public static final String Section=Parent+"Section";
    public static final String ProductId=Parent+"product/id";
    public static final String PlaceOrder=Parent+"placeorder";
    public static final String Order=Parent+"order";
    public static final String OrderId=Parent+"order/id";
    public static final String ChangePassword=Parent+"changepassword";
    public static final String SalesPerson=Parent+"SalesPerson";
    public static final String Location=Parent+"Location";
    public static final String TrackLocation=Parent+"TrackLocation";
}
